...
**3.** Un número nuevo no sirve para recuperar la clave hasta que alguien lo habilita, y no hay
espera por tiempo. Quien cambia su número lo carga y queda a la espera; la habilitación le aparece
en las tareas pendientes de quien lo tiene a cargo, que se comunica con esa persona para verificar
que el cambio es real. A un Asistente y a una Familia los habilita la coordinación; a quien
coordina lo habilita la administración de la Prestadora. Mientras tanto la persona entra igual con
su correo y su clave: lo único que no puede es usar ese número para recuperarla.

**4.** El mismo número de teléfono en dos personas: se permite si es un número de línea fija y se
rechaza si es un número de celular. Una casa con una sola línea es lo corriente; un celular es de
una sola persona. Hoy la base lo acepta en todos los casos, porque lo único que controla es que la
misma persona no cargue dos veces el mismo número. Hace falta distinguir línea fija de celular y
una migración que impida repetir un celular.
...
**6.** El mensaje de texto: se deja hecho el lugar y apagado, sin contratar ningún proveedor. Se
escribe la parte que le toca a Careonys —la vía existe, sale en la lista y no se puede elegir
mientras no haya proveedor cargado—, de modo que encenderla sea cargar una configuración y nada
más. Entra como respaldo de WhatsApp, para quien no lo usa, y con el aviso de que es la vía débil.
...
**8.** Terminar de acomodar la pantalla de la Familia al hueco que deja el saldo. El motor ya no
...
**9.** El intermediario que recibe en bloque y reparte entra sólo en prestación directa, como un
...
**11.** Qué contesta sola la respuesta automática de WhatsApp: todo menos salud y urgencias, y
...
**20.** La pantalla del formulario, que es lo único que falta: la base y el motor ya guardan y comprueban los campos de las seis secciones de `docs/PRD_03_Reclutamiento.md`, y el motor entrega las listas de opciones en `GET /api/publico/:prestadora/postulacion-asistente/opciones`. Se dibuja desde la declaración, no a mano. Esperaba el paso anterior.
...
**28. Usted** — La alerta por salida del domicilio, dos decisiones que no puedo tomar yo. Hoy la cuenta se hace con una velocidad media única y distancia en línea recta (`backend/src/utils/llegadaEstimada.js:39-45`), se dispara recién cuando alguien marcó la salida, y mide llegada tarde, no que el Asistente siga en su casa.
...
**40.** Armarlo. La subida del certificado a un depósito de archivos ya quedó resuelta con el
...
**52.** Sacar el nombre viejo `aurevia` de adentro del producto. **Se decide y se hace con la
...
**61. Usted** — El alta y la baja de Prestadoras: Match expone una puerta firmada para que CeltaTech dé de alta y de baja clientes; Careonys no tiene nada parecido. ¿Se construye la misma, o se siguen dando de alta a mano?

**62. Usted** — El acompañamiento online: ¿prestación más, o guardia sin domicilio? Si es guardia, hay que decidir qué reemplaza al check-in por ubicación.
...
**67. Usted** — ¿Se autoriza construir `careonys.com` según `docs/PRD_01_Sitio_Web.md`? El PRD ya está entero. Hoy `sitio-web/` es una página que dice «En construcción». **El diseño se hace de cero**: del sitio público de Match no viaja nada visual.
